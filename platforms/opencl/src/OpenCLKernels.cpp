    ...
    if (force.getNonbondedMethod() == NonbondedForce::Ewald) {
        ...
        if (cl.getContextIndex() == 0) {
            ...
            if (pmeio == NULL) {
                ...
                int energyElementSize = (cl.getUseDoublePrecision() ? sizeof(double) : sizeof(float));
...
double OpenCLCalcNonbondedForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy, bool includeDirect, bool includeReciprocal) {
    ...
    if (!hasInitializedKernel) {
        ...
        if (pmeGrid != NULL) {
            ...
            pmeConvolutionKernel.setArg<cl::Buffer>(1, usePmeQueue ? pmeEnergyBuffer->getDeviceBuffer() : cl.getEnergyBuffer().getDeviceBuffer());
